...
void drawHeart(int x, int y, int size, int color) {
    ...
    drawCircle(x + radius, y - radius, radius, color, true);
    // triangle 
    // Draw the inverted triangle at the bottom of the heart
    int x0 = x;
    int y0 = y + radius;
    drawInvertedTriangle(x0, y0 - 1, triangleHeight, color, true);
    // drawTriangle(int x0, int y0, int height, int color, bool fill)
    // drawTriangle(x - size / 2, y - radius, triangleHeight, color, true);
}

void drawInvertedTriangle(int x0, int y0, int height, int color, bool fill) {
    // Calculate the side length using integer approximation
    // s = (2 * h) / sqrt(3)
    // Approximate sqrt(3) as 1732/1000
    int size = (2 * height * 1000) / 1732;
    ...
    // the coordinates of the vertices
    int x1 = x0;
    int y1 = y0 + (2 * height) / 3; // Bottom vertex

    int x2 = x0 - size / 2;
    int y2 = y0 - height / 3; // Top left vertex

    int x3 = x0 + size / 2;
    int y3 = y0 - height / 3; // Top right vertex
